 ...
 * Infobox styles
 */
function aawp_settings_display_infoboxes( $plugin_slug ) {
 ...
 * Support
 */
function aawp_settings_infobox_support( $plugin_slug ) {

    // Debug
    //$plugin_slug = 'AAWP_BESTSELLER';

    $title = __( 'Resources & Support', 'aawp' );
    $header = aawp_get_settings_infobox_support_header( $plugin_slug );
    $links = aawp_get_settings_infobox_support_links( $plugin_slug );
    ...
    <?php if ( in_array( $plugin_slug, array( 'AAWP_DEV', 'AAWP_BETA', 'AAWP_AFFILIATE' ) ) ) { ?>
...
function aawp_get_settings_infobox_support_header( $plugin_slug ) {
    ...
    if ( 'AAWP_BESTSELLER' === $plugin_slug ) {

        $header = array(
            'img' => ( aawp_is_lang_de() ) ? 'https://cdn.flowdee.de/aawp/assets/aawp-banner-de-infobox-envato-discount-480x200.gif' : 'https://cdn.flowdee.de/aawp/assets/aawp-banner-en-infobox-envato-discount-480x200.gif',
            'url' => ( aawp_is_lang_de() ) ? 'https://aawp.de/envato/?utm_source=codecanyon&utm_medium=Amazon%20Bestseller%20for%20WordPress&utm_content=Plugin%20infobox&utm_campaign=CS' : 'https://getaawp.com/envato/?utm_source=codecanyon&utm_medium=Amazon%20Bestseller%20for%20WordPress&utm_content=Plugin%20infobox&utm_campaign=CS'
        );
    }
...
function aawp_get_settings_infobox_support_links( $plugin_slug ) {

    $links = array();

    if ( 'AAWP_BESTSELLER' === $plugin_slug ) {

        $links['https://kryptonitewp.com/downloads/amazon-bestseller-wordpress/'] = __( 'Plugin Website', 'aawp' );
        $links['https://codecanyon.net/item/amazon-bestseller-for-wordpress/10859045?ref=flowdee'] = __( 'Plugin on CodeCanyon', 'aawp' );
        $links['https://kryptonitewp.com/support/knb/amazon-bestseller-wordpress-documentation/'] = __( 'Documentation', 'aawp' );
        $links['https://kryptonitewp.com/downloads/amazon-bestseller-wordpress/'] = __( 'Changelog', 'aawp' );
        $links['https://kryptonitewp.com/support/'] = __( 'Support', 'aawp' );
        $links['https://twitter.com/flowdee'] = __( 'Follow me on Twitter', 'aawp' );
...
function aawp_settings_infobox_newsletter( $plugin_slug ) {
...
function aawp_settings_infobox_translations( $plugin_slug ) {

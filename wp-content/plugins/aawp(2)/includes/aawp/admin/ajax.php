...
function aawp_admin_ajax_search() {
    ...
    if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
        ...
        if ( ! empty( $type ) && ! empty( $keywords ) ) {
            ...
            $results = $API_Handler->get_search_results( $search_args );
            ...
            // Array of ASINs
            } elseif ( is_array( $results ) && sizeof( $results ) > 0 ) {

                $products = $API_Handler->get_products( $results, array( 'crawl_reviews' => 0 ) );

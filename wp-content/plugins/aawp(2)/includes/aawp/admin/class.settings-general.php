    ...
    class AAWP_Settings_General extends AAWP_Functions {
        ...
        public function add_settings() {
            ...
            add_settings_field(
                'aawp_amp_support',
                __( 'AMP Support', 'aawp' ),
                array( &$this, 'settings_amp_support_render' ),
                'aawp_general',
                'aawp_general_section',
                array('label_for' => 'aawp_amp_support')
            );
        ...
        public function settings_shortcode_render() {
            ...
            <select id="aawp_shortcode" name="aawp_general[shortcode]">
                <?php foreach ( $shortcode_options as $value => $label ) { ?>
                    <option value="<?php echo $value; ?>" <?php selected( $this->shortcode, $value ); ?>><?php echo $label; ?></option>
                <?php } ?>
            </select>
            <?php
        }
        ...
        // AMP Support
        public function settings_amp_support_render() {

            $amp_support = ( isset ( $this->options['general']['amp_support'] ) && $this->options['general']['amp_support'] == '1' ) ? 1 : 0;

            ?>
            <input type="checkbox" id="aawp_amp_support" name="aawp_general[amp_support]" value="1" <?php echo($amp_support == 1 ? 'checked' : ''); ?>>

            <label for="aawp_amp_support"><?php printf( wp_kses( __( 'Check to activate <a href="%s" target="_blank">Accelerated Mobile Pages (AMP)</a> support', 'aawp' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( 'https://wordpress.org/plugins/amp/' ) ); ?></label>

            <p><small><?php _e('<strong>Note:</strong> In order to use this feature, PHP <code>allow_url_fopen</code> setting must be enabled on your server.', 'aawp' ); ?></small></p>
            <?php
        }

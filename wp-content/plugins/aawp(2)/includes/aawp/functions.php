 ...
 */
function aawp_get_amp_status() {

    $options_general = get_option( 'aawp_general' );

    $amp_support = ( isset ( $options_general['amp_support'] ) && $options_general['amp_support'] == '1' ) ? 1 : 0;
 ...
 * @return mixed|null
 */
function aawp_replace_url_tracking_id_placeholder( $url, $tracking_id ) {
 ...
 */
function aawp_is_amp() {

    if ( ! aawp_get_amp_status() )
        return false;
...
function aawp_get_credits_link() {
    ...
    $link = sprintf( wp_kses( __( 'This product was presentation was made with <a href="%s" rel="nofollow" target="_blank" title="Amazon Affiliate WordPress Plugin">AAWP</a> plugin.', 'aawp' ), array(  'a' => array( 'href' => array(), 'rel' => array(), 'target' => array(), 'title' => array() ) ) ), esc_url( $url ) );
 ...
 * @return bool
 */
function aawp_crawling_ratings_activated() {
...
/**
 * Get API error message text from code or string
 *
 * @param $error_message
 *
 * @return mixed|string|void
 */
function aawp_get_api_error_message_text($error_message) {

    // RequestThrottled

    if (strpos($error_message, 'MissingClientTokenId') !== false) {
        return __('API key is missing.', 'aawp');

    } elseif (strpos($error_message, 'InvalidClientTokenId') !== false) {
        return __('The provided API key does not exist.', 'aawp');

    } elseif (strpos($error_message, 'SignatureDoesNotMatch') !== false) {
        return __('The provided API secret is not valid for the given API key.', 'aawp');

    } elseif (strpos($error_message, 'AWS.InvalidAccount') !== false) {
        return __('Your API key is not registered for the Amazon Product Advertising API.', 'aawp');

    } elseif (strpos($error_message, 'AWS.InvalidAssociate') !== false) {
        return __('The provided API key is either not registered as an Amazon Associate or for using the Amazon Product Advertising API.', 'aawp');

    } else {
        return 'Code: "' . $error_message . ' - ' . __('Something went wrong. Please check your API keys', 'aawp');
    }
}
 ...
 * @return array
 */
function aawp_get_amazon_image_sources() {

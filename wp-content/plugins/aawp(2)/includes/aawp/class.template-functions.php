...
if ( ! class_exists( 'AAWP_Template_Functions') ) {
    ...
    class AAWP_Template_Functions extends AAWP_Functions {
        ...
        private function locate_template( $template_names, $load = false, $require_once = true ) {
            ...
            $template_stack[] = trailingslashit( $aawp_plugin_path . 'templates/parts' );
            $template_stack[] = trailingslashit( $aawp_plugin_path . 'templates/products' );
            $template_stack[] = trailingslashit( $aawp_plugin_path . 'templates' );

            // check plugin directory for deprecated templates as well
            $template_stack[] = trailingslashit( $aawp_plugin_path . 'templates/deprecated' );
        ...
        public function get_product_url( $type = 'basic', $origin = 'default' ) {
            ...
            $url = aawp_replace_url_tracking_id_placeholder( $url, $tracking_id );
        ...
        public function get_product_image( $size = '' ) {
	        ...
	        if ( $custom_image && strpos( $custom_image, 'images-amazon.com') === false ) {
		        return $custom_image;
	        }
        	...
        	if ( ! $image ) {
		        ...
		        if ( empty( $image ) )
			        return apply_filters( 'aawp_func_product_image_placeholder', $this->plugin_url . 'public/assets/img/placeholder-' . $size . '.jpg' );
	        }
	        ...
	        if ( '1' == aawp_get_option( 'image_proxy', 'output' ) && ini_get('allow_url_fopen') ) {
		        ...
		        $image = add_query_arg( array(
			        'url' => rawurlencode( base64_encode( $image ) )
        ...
        public function get_product_price( $type = 'display', $format = 'formatted' ) {
            ...
            // Convert deprecated pricing types
            if ( 'old' === $type )
                $type = 'list';
            ...
            $price = $this->item->get_price( $type, $return_formatted );
        ...
        public function get_product_price_saving( $formatted = true ) {
            return $this->item->get_price_saving( $formatted );
        }
        ...
        public function get_product_price_saving_percentage( $formatted = true ) {
            return $this->item->get_price_saving_percentage( $formatted );
        }
        ...
        public function the_product_check_prime_logo($echo = true) {
            ...
            $prime_logo = aawp_get_prime_check_logo( $this->api_country, $this->api_associate_tag, $this->atts );

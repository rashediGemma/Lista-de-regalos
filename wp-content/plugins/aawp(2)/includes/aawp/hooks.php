...
add_filter('the_content', 'aawp_maybe_cleanup_shortcode_output' );
...
function aawp_api_response_validation_error( $error ) {
    ...
    if ( isset( $api_options['status'] ) && '1' == $api_options['status'] ) {
        ...
        // Add Log
        aawp_add_log( sprintf( esc_html__( 'Disconnected from Amazon API: %s', 'aawp' ), aawp_get_api_error_message_text( $error['Code'] ) . ' ( Code: ' . $error['Code'] . ')' ) );

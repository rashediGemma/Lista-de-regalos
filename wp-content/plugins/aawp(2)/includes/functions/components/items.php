...
add_filter( 'aawp_caching_key', 'aawp_items_extend_list_key', 10, 4 );
 ...
 * @return int
 */
function aawp_items_update_query_items( $query_items, $atts ) {
...
add_filter( 'aawp_list_query_items', 'aawp_items_update_query_items', 10, 2 );
    ...
    // Case: Pricing
        ...
        foreach ( $items as $item_id => $item ) {
            ...
            // Percentage saved
                ...
                $value = $AAWP_Product->get_price_saving_percentage( false );

            // Amount saved
            } elseif ( 'amount_saved' === $orderby ) {
                $value = $AAWP_Product->get_price_saving( false );
            }
        ...
        foreach ( $items as $item_id => $item ) {
            ...
            // Price: Specials
                ...
                if ( in_array( 'offer', $filter ) ) {
                    ...
                    if ( 0 == $AAWP_Product->get_price_saving_percentage() ) {
                        $unset = true;
                    }
                }
                ...
                if ( in_array( 'available', $filter ) ) {
                    ...
                    if ( 0 == $AAWP_Product->get_price( 'display', false ) || 'now' != $AAWP_Product->get_availability() ) {
                        $unset = true;
                    }
                }
